...
using namespace std;
int area(int u,int v,int w,int x,int y,int z)
{
    int ans= abs((u*x+w*z+y*v)-(v*w+x*y+z*u))/2;
    return ans;
}
int main()
{
    ...
    while(m--)
    {
        int x,y;
        cin>>x>>y;
        int a,b,c,dd,e,f,g,h;
        a=0,b=d,c=d,dd=0,e=n,f=n-d,g=n-d,h=n;
        int rectangle=(n-d)*d;
        int ans1=area(a,b,c,dd,n,d);
        int ans2=area(a,b,e,f,n,d);
        int ans3=area(c,dd,g,h,n,d);
        int ans4=area(e,f,g,h,n,d);
        int ans=ans1+ans2+ans3+ans4;
        cout<<ans1<<" "<<ans2<<" "<<ans3<<" "<<ans4<<" "<<rectangle<<nl;
        //cout<<(ans==rectangle ? "YES" : "NO")<<nl;
