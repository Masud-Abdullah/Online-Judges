...
int main()
{
    ...
    while(t--)
    {
        ...
        for(ll i=0;i<n;i++)
        {
            ...
            if(a[i].second>=0)y++;
            if(a[i].second<=0)z++;
            // if(a[i].first>0 && a[i].second>0)w=1;
            // else if(a[i].first>0 && a[i].second<0)x=1;
            // else if(a[i].first<0 && a[i].second<0)y=1;
            // else if(a[i].first<0 && a[i].second>0)z=1;
        }

        if(z==n || y==n || x==n || w==n)YY;
        else NN;

        // if(w==1 && x==1 && y==0 && z==0)YY;
        // else if(x==1 && y==1 && z==0 && w==0)YY;
        // else if(y==1 && z==1 && w==0 && x==0)YY;
        // else if(z==1 && w==1 && x==0 && y==0)YY;
        // else if((x==0 && y==0 && z==0)||(w==0&&x==0&&y==0)||(w==0 &&x==0&&z==0)||(w==0&&y==0&&z==0))YY;
        // else NN;
